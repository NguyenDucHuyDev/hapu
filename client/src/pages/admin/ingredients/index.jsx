...
const listIngredients = () => {
  ...
  /**
   * @type {Array<(import("../../../../type").ingredientsInfo),Function>}
   */
  ...
  const [columns] = useState([
    ...
    },
    {
      ...
      key: "id",
      render(data, item) {
        return <Link to={`/admin/ingredients/${item.slug}-${item.id}/edit`} className="text-blue-500">Chỉnh sửa</Link>
      }
    },
  ]);
  ...
  const getData = async (page, per_page) => {
    ...
    setListIngredients([...res.data.data])
  }
  useEffect(() => {
    getData()
  }, [])
  ...
  return (
    <>
      <Divider className="!text-lg" orientation="left">Danh sách hoạt chất</Divider>
      <Table
